...
if(isset($_GET["field"]))
{
	...
	$field_self->get_by_id_category_field($GLOBALS['con'],$field);
    $field_groups = new web_field_group();
    $arrfieldgroups = $field_groups->GetAll($GLOBALS['con']);
// Denisi
...
if($_POST)
{
	...
	$kot = $cat_field->get_by_id_category_field($GLOBALS['con'],$_POST["id_category_field"]);	
    $kot->field_label=$_POST["field_label"];
    $kot->field_order=$_POST["field_order"];
    $kot->id_group=$_POST["id_group"];
    $kot->Update($GLOBALS['con']);
    /**********/
