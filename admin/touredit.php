<?php
	...
	$post->post_image="http://placehold.it/300x300.png";
	if($id){
	...
	if($_POST){
		...
		$termrels = $_POST["kat"];
		//delete previous
		$res = mysql_query("DELETE FROM web_termrel wtr inner join web_term t on wtr.id_term=t.id_term where t.term_type='category' and id_post='".$id."'");
		...
		$res = mysql_query("DELETE FROM web_termrel wtr inner join web_term t on wtr.id_term=t.id_term where t.term_type='tag' and id_post='".$id."'");
		$termrels = $_POST["tags"];
		...
		$postmeta = new web_postmeta();
		$postmeta = $postmeta->get_by_unique($id,"price");
		$postmeta->post_id = $id;
		$postmeta->meta_key = "price";
		$postmeta->meta_value = $_POST["price"];
		if($postmeta->meta_id) $postmeta->Update(); else $postmeta->Insert();
		
		$postmeta = new web_postmeta();
		$postmeta = $postmeta->get_by_unique($id,"destination");
		$postmeta->post_id = $id;
		$postmeta->meta_key = "destination";
		$postmeta->meta_value = $_POST["destination"];
		if($postmeta->meta_id) $postmeta->Update(); else $postmeta->Insert();

		$postmeta = new web_postmeta();
		$postmeta = $postmeta->get_by_unique($id,"duration");
		$postmeta->post_id = $id;
		$postmeta->meta_key = "duration";
		$postmeta->meta_value = $_POST["duration"];
		if($postmeta->meta_id) $postmeta->Update(); else $postmeta->Insert();
		
		$postmeta = new web_postmeta();
		$postmeta = $postmeta->get_by_unique($id,"stars");
		$postmeta->post_id = $id;
		$postmeta->meta_key = "stars";
		$postmeta->meta_value = $_POST["stars"];
		if($postmeta->meta_id) $postmeta->Update(); else $postmeta->Insert();
		
		/**********/
		header("Location:touredit.php?id=".$post->id_post);die();
	}
		...
		<div class="container">
			<form method='post' action='' role="form" class="" enctype="multipart/form-data">
				...
				<div class="form-group col-md-12">
					...
					<a href="touredit.php" class="btn btn-info"><span class="glyphicon glyphicon-plus"></span> Krijo tjeter</a>
					<a href="tourlist.php" class="btn btn-info"><span class="glyphicon glyphicon-back"></span> Kthehu te lista</a>
				...
				<div class="col-md-3">
					<div class="form-group">
					<label for="img">Imazhi i tur-it</label>
				...
				<div class="form-group">
					...
					<select class="form-control" id="parent" name="parent">
						...
						<?php
							$posts = $post->Find("post_type='post' order by post_order");
							foreach($posts as $p){
								$selected ="";
				...
				<div class="form-group">
					<label for="sidebar">Paneli anesor</label>
				...
				<div class="form-group">
					<label for="publish-date">Data e publikimit</label>
				...
				<div class="form-group">
					...
					<input type="number" class="form-control" id="price" name="price" placeholder="Cmimi" value="<?php $postmeta = $postmeta->get_by_unique($id,"price"); echo $postmeta->meta_value;?>">
				</div>
				<div class="form-group">
					<label for="destination">Destinacioni</label>
					<input type="text" class="form-control" id="destination" name="destination" placeholder="Destinationi" value="<?php $postmeta = $postmeta->get_by_unique($id,"destination"); echo $postmeta->meta_value;?>">
				</div>
				<div class="form-group">
					<label for="duration">Kohezgjatja</label>
					<input type="text" class="form-control" id="duration" name="duration" placeholder="Kohezgjatja" value="<?php $postmeta = $postmeta->get_by_unique($id,"duration"); echo $postmeta->meta_value;?>">
				</div>
				<div class="form-group">
					<label for="stars">Yje</label>
					<input type="number" class="form-control" id="stars" name="stars" placeholder="Yje" value="<?php $postmeta = $postmeta->get_by_unique($id,"stars"); echo $postmeta->meta_value;?>">
				</div>
				<?php if($id){?>
